...
(let ((cur-dir-path (uiop:pathname-directory-pathname *load-truename*))
  ...
  (handler-case
    (handler-bind ((warning #'(lambda (warning-in)
                                (let ((warning-str-in (format nil "~A" warning-in)))
                                  (if (and (search "redefining" warning-str-in :test #'char-equal)
                                           (search "in DEFMACRO" warning-str-in :test #'char-equal))
                                    (format t "~&[33mBenign warning[0m: ~A~%" warning-in)
